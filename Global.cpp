...
static BOOL TrayMessage(HWND hWnd, DWORD dwMessage, UINT uID, HICON hIcon, LPCTSTR lpszTip)
{
   ...
   if (lpszTip)
   {
	   ...
	   lstrcpyn(tnd.szTip, lpszTip, nSize);
   }
   else
      tnd.szTip[0] = _T('\0');
...
{
   return TrayMessage(hWnd, NIM_ADD, uID, hIcon, lpszTip);
}


BOOL Tray_NotifyModify(HWND hWnd, UINT uID, HICON hIcon, LPCTSTR lpszTip)
{
   return TrayMessage(hWnd, NIM_MODIFY, uID, hIcon, lpszTip);
}
...
/// @param  
/// @return 
/// @date   Tuesday, September 02, 2008  4:00:48 PM
////////////////////////////////////////////////////////////////////////////////////////////////////
...
BOOL GetFileCRC(LPCTSTR szFileName, DWORD &dwResult,int nStartPos)
{
#define FILEBUFSIZE 4096
	HANDLE	hf;
	DWORD	dwRead;
	BYTE	buffer[FILEBUFSIZE];
	DWORD	dwCRC;

	hf = CreateFile(szFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	if(hf==INVALID_HANDLE_VALUE) {ASSERT(0); return FALSE;}

	SetFilePointer(hf, nStartPos, (PLONG) NULL, FILE_BEGIN);

	dwCRC = 0;
	while(ReadFile(hf, &buffer, sizeof(buffer), &dwRead, NULL))
	{
		if(dwRead==0) break;
		dwCRC = CRC32(dwCRC, buffer, dwRead);
	}
	CloseHandle(hf);
